...
## 2026-05-31 - Role/provenance contract adoption draft

- Operator instruction: role lanes need simple replayable steps such as `design -> apply -> verify` and `input -> analysis -> output`, and interaction provenance needs slug/path/hash/signature/parents for KG traceability.
- Repair performed: drafted `docs/ROLE_AND_PROVENANCE_CONTRACT.md` as reusable gsigmad machinery, added a deterministic worked receipt in `examples/interaction_receipts/`, and added `tests/test_role_provenance_contract.py`.
- Role loops captured: PI `input -> analysis -> interpretation`; PM `scope -> sequence -> gate`; SWE `design -> apply -> verify`; Review `input -> analysis -> verdict`; Operator `request -> approve_or_deny -> receipt`.
- Content hash: `sha256:32fb59ec83d7dcc68ae4b21c62754fe81bf2b8ad7d9305c8cac56392b046a848`.
- Boundary: draft governance vocabulary only. No live writeback, import, push, external database mutation, claim promotion, or canonical truth promotion was performed.
